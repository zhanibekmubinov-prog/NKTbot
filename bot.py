...
Ключевое:
• Меню всегда доступно (нативная кнопка Telegram + /start /menu /cancel; любое сообщение
  в простое открывает меню). ВНУТРИ диалога текст обрабатывают шаги (allow_reentry=False,
  иначе catch-all сбрасывал бы ввод).
• На неверный ввод бот сразу отвечает подсказкой (fallback), а не молчит.
• Передвижение и списание идут ОТ ЛОКАЦИИ: сначала выбираешь существующую локацию,
  затем бот показывает только реально имеющиеся там статусы → виды → размеры → резьбы.
  Получатель (куда) — с проверкой имени и подсказкой Клода; новую заводим по согласию.
• Поступление — из справочника (можно вводить новые размеры/резьбы).

ENV: TELEGRAM_TOKEN, GOOGLE_SHEET_ID, GOOGLE_CREDENTIALS, ANTHROPIC_API_KEY (необязателен).
"""
...
(MENU,
 MV_FROMSTAT, MV_PART, MV_SIZE, MV_THREAD, MV_TOSTAT, MV_QTY, MV_CONF,
 IN_SUP, IN_PART, IN_SIZE, IN_THREAD, IN_STAT, IN_QTY, IN_CONF,
 WO_STAT, WO_PART, WO_SIZE, WO_THREAD, WO_QTY, WO_REASON, WO_CONF,
...
async def menu_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if act == 'move':
        return await ask_location(update, context, 'mv_a', "🔄 Передвижение.\nОткуда (локация А)?", allow_new=False)
    if act == 'in':
        await show(update, "📥 Поступление.\nВведите поставщика:")
        return IN_SUP
    if act == 'woff':
        return await ask_location(update, context, 'wo', "🗑 Списание.\nС какой локации?", allow_new=False)
    if act == 'loc':
...
async def loc_typein(update, context):
    ...
    if cands:
        ...
        await show(update, f"Локации «{typed}» нет. Создать новую?", kb([], extra=extra, context=context))
    else:
        await show(update, f"❌ Локации «{typed}» нет в списке. Выберите существующую или введите заново.",
...
async def loc_pick(update, context):
    ...
    if data == 'loc_back':
        return await ask_location(update, context, context.user_data['loc_after'],
                                  context.user_data['loc_title'], context.user_data.get('loc_allow_new', True))
    if data == 'loc_create':
...
# ═══════════════ ПЕРЕДВИЖЕНИЕ (от локации, из остатков) ═══════════════

async def mv_ask_fromstat(update, context):
    d = context.user_data['draft']
    sts = ENG.statuses_at(d['loc_a'])
    if not sts:
        await show(update, f"❌ На «{d['loc_a']}» ничего не числится.")
        return await ask_location(update, context, 'mv_a', "Откуда (локация А)?", allow_new=False)
    await show(update, f"Статус на «{d['loc_a']}» (что там есть)?", kb(sts, context=context))
    return MV_FROMSTAT


async def mv_fromstat(update, context):
    d = context.user_data['draft']; d['status_a'] = picked(update, context)
    await show(update, "Вид оборудования?", kb(ENG.parts_at(d['loc_a'], d['status_a']), context=context))
    return MV_PART


async def mv_part(update, context):
    d = context.user_data['draft']; d['part'] = picked(update, context)
    await show(update, f"Размер ({d['part']})?", kb(ENG.sizes_at(d['loc_a'], d['status_a'], d['part']), context=context))
    return MV_SIZE


async def mv_size(update, context):
    d = context.user_data['draft']; d['size'] = picked(update, context)
    if d['part'] == 'НКТ':
        thr = ENG.threads_at(d['loc_a'], d['status_a'], d['part'], d['size'])
        if thr:
            await show(update, "Резьба?", kb(thr, context=context))
            return MV_THREAD
    d['thread'] = ''
    return await ask_location(update, context, 'mv_b', "Куда (локация Б)?", allow_new=True)


async def mv_thread(update, context):
    context.user_data['draft']['thread'] = picked(update, context)
    return await ask_location(update, context, 'mv_b', "Куда (локация Б)?", allow_new=True)
...
async def mv_qty(update, context):
    ...
    warn = ''
    if q > avail:
        warn = (f"⚠️ На «{d['loc_a']}» ({d['status_a']}) числится {avail} шт "
...
# ═══════════════ ПОСТУПЛЕНИЕ (из справочника, можно новое) ═══════════════

async def in_supplier(update, context):
    context.user_data['draft']['supplier'] = update.message.text.strip()
    await show(update, "Вид оборудования?", kb(ENG.parts(), context=context))
    return IN_PART


async def in_part(update, context):
    p = picked(update, context); context.user_data['draft']['part'] = p
    await show(update, f"Размер ({p})?",
               kb(ENG.sizes_for(p), extra=[('✏️ Другой (ввести)', 'size_manual')], context=context))
    return IN_SIZE
...
async def in_size(update, context):
    ...
    if d['part'] == 'НКТ':
        await show(update, "Резьба (НКТ)?",
                   kb(ENG.threads_for(d['part']), extra=[('✏️ Другая (ввести)', 'thread_manual')], context=context))
        return IN_THREAD
    d['thread'] = ''
    return await ask_location(update, context, 'in', "Локация?", allow_new=True)
...
# ═══════════════ СПИСАНИЕ (от локации, из остатков) ═══════════════

async def wo_ask_stat(update, context):
    d = context.user_data['draft']
    sts = ENG.statuses_at(d['location'])
    if not sts:
        await show(update, f"❌ На «{d['location']}» ничего не числится.")
        return await ask_location(update, context, 'wo', "С какой локации списываем?", allow_new=False)
    await show(update, f"Статус на «{d['location']}»?", kb(sts, context=context))
    return WO_STAT


async def wo_stat(update, context):
    d = context.user_data['draft']; d['status'] = picked(update, context)
    await show(update, "Вид оборудования?", kb(ENG.parts_at(d['location'], d['status']), context=context))
    return WO_PART


async def wo_part(update, context):
    d = context.user_data['draft']; d['part'] = picked(update, context)
    await show(update, f"Размер ({d['part']})?", kb(ENG.sizes_at(d['location'], d['status'], d['part']), context=context))
    return WO_SIZE
...
async def wo_size(update, context):
    d = context.user_data['draft']; d['size'] = picked(update, context)
    if d['part'] == 'НКТ':
        thr = ENG.threads_at(d['location'], d['status'], d['part'], d['size'])
        if thr:
            await show(update, "Резьба?", kb(thr, context=context))
            return WO_THREAD
    d['thread'] = ''
...
async def wo_reason(update, context):
    d = context.user_data['draft']; d['reason'] = update.message.text.strip()
    avail = ENG.available(d['location'], d['status'], d['part'], d['size'], d.get('thread') or '')
    warn = ''
    if d['qty'] > avail:
        warn = (f"⚠️ На «{d['location']}» ({d['status']}) числится {avail} шт "
...
# ═══════════════ подсказки на неверный ввод ═══════════════

async def need_button(update, context):
    await update.message.reply_text("⚠️ Здесь нужно выбрать вариант кнопкой ниже. "
                                    "Отмена — /cancel, меню — /menu.")
    return None  # остаёмся в текущем шаге


async def stale_cb(update, context):
    await update.callback_query.answer("Кнопка устарела. Откройте меню: /menu", show_alert=True)
    return None
...
async def post_init(app):
    await app.bot.set_my_commands([BotCommand('start', 'Меню'), BotCommand('menu', 'Меню'),
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[CommandHandler('start', start), CommandHandler('menu', start),
                      ...
                      CallbackQueryHandler(start)],  # срабатывают только когда диалога нет
        states={
            ...
            MV_SIZE: [cq(mv_size, r'^opt:')],
            MV_THREAD: [cq(mv_thread, r'^opt:')],
            ...
            WO_SIZE: [cq(wo_size, r'^opt:')],
            WO_THREAD: [cq(wo_thread, r'^opt:')],
            WO_QTY: [txt(wo_qty)],
        ...
        fallbacks=[CommandHandler('cancel', cancel),
                   ...
                   CallbackQueryHandler(stale_cb),            # прочие устаревшие кнопки
                   MessageHandler(filters.TEXT & ~filters.COMMAND, need_button)],  # текст вместо кнопки
        allow_reentry=False,
    )
