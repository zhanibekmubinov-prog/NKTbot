...
• Виды/размеры/резьбы/статусы/локации берутся из «Справочника» и «Локаций» → кнопки,
  поэтому опечатки при выборе исключены.
• Claude Haiku используется ТОЛЬКО как подсказка по названию локации, когда человек
  вводит её вручную и локальное сравнение (difflib) не уверено. Если имя новое — заводим.

Действия: Передвижение · Поступление · Списание · Добавить локацию · Добавить вид · Остатки.
...
# ── состояния ──
(MENU,
 MV_FROM, MV_FROMSTAT, MV_TO, MV_TOSTAT, MV_PART, MV_SIZE, MV_THREAD, MV_QTY, MV_CONF,
 IN_SUP, IN_PART, IN_SIZE, IN_THREAD, IN_LOC, IN_STAT, IN_QTY, IN_CONF,
 WO_PART, WO_SIZE, WO_THREAD, WO_STAT, WO_QTY, WO_REASON, WO_CONF,
 AL_NAME, AL_TYPE, AL_CONF,
 AR_PART, AR_SIZE, AR_THREAD, AR_CONF,
 INV_PART, INV_LOC,
 LOC_CHOOSE, LOC_TYPEIN, LOC_PICK) = range(37)

# ═══════════════ утилиты интерфейса ═══════════════

def kb(options, extra=None, cols=2, cancel=True, context=None):
    """Клавиатура из options (callback 'opt:i') + доп. кнопки + Отмена. Сохраняет options в user_data."""
...
async def show(update, text, markup=None):
    """Ответить и в callback, и в обычном сообщении."""
...
def picked(update, context):
    """Вернуть выбранный текст по callback 'opt:i'."""
...
async def menu_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if act == 'move':
        return await ask_location(update, context, after='mv_a', title="🔄 Передвижение.\nОткуда (локация А)?")
    if act == 'in':
...
# ═══════════════ ВЫБОР ЛОКАЦИИ (общий, с гибридным сопоставлением) ═══════════════

async def ask_location(update, context, after, title):
    context.user_data['loc_after'] = after
    names = ENG.location_names()
    await show(update, title + "\nВыберите из списка или «Другая»:",
               kb(names, extra=[('✏️ Другая (ввести)', 'loc_manual')], context=context))
    return LOC_CHOOSE
...
async def loc_typein(update, context):
    ...
    if len(cands) < 2 and AI:
        extra = claude_suggest_location(typed, ENG.location_names())
        for e in extra:
            if e not in cands:
                cands.append(e)
    extra_btns = [(f'➕ Создать: «{typed}»', 'loc_create'), ('✏️ Ввести заново', 'loc_manual')]
    if cands:
        await show(update, f"Не нашёл точного совпадения для «{typed}». Вы имели в виду:",
                   kb(cands, extra=extra_btns, cols=1, context=context))
    else:
        await show(update, f"Локации «{typed}» нет. Создать новую?",
                   kb([], extra=extra_btns, context=context))
    return LOC_PICK
...
def claude_suggest_location(typed, names):
    """Спросить у Haiku, какая из существующих локаций имелась в виду. Вернёт список (0-2)."""
    if not AI:
        return []
    try:
        ...
        arr = json.loads(txt)
        return [n for n in arr if n in names][:2]
    except Exception:
        log.exception("claude_suggest_location")
        return []


# ═══════════════ выбор вида/размера/резьбы (общие помощники) ═══════════════

async def ask_part(update, context, state_prompt):
    await show(update, state_prompt, kb(ENG.parts(), context=context))


async def ask_size(update, context, part):
    sizes = ENG.sizes_for(part)
    await show(update, f"Размер ({part})?",
               kb(sizes, extra=[('✏️ Другой (ввести)', 'size_manual')], context=context))


async def ask_thread(update, context, part):
    thr = ENG.threads_for(part)
    await show(update, "Резьба (НКТ)?",
               kb(thr, extra=[('✏️ Другая (ввести)', 'thread_manual')], context=context))
...
async def mv_fromstat(update, context):
    ...
    return await ask_location(update, context, after='mv_b', title="Куда (локация Б)?")
...
async def mv_qty(update, context):
    ...
    txt = (f"Проверьте передвижение:\n"
           f"• {d['part']} {d['size']}{th} — {q} шт\n"
...
async def in_size(update, context):
    ...
    if d['part'] == 'НКТ':
        ...
        return IN_THREAD
    d['thread'] = ''
    return await ask_location(update, context, after='in', title="Локация?")
...
async def wo_size(update, context):
    ...
    if d['part'] == 'НКТ':
        ...
        return WO_THREAD
    d['thread'] = ''
    return await ask_location(update, context, after='wo', title="С какой локации списываем?")
...
async def wo_reason(update, context):
    ...
    th = f" {d['thread']}" if d.get('thread') else ''
    txt = (f"Проверьте списание:\n"
           f"• {d['part']} {d['size']}{th} — {d['qty']} шт\n"
           f"• {d['location']} ({d['status']})\n"
           f"• Причина: {d['reason']}")
    await show(update, txt, kb([], extra=[('✅ Записать', 'ok')], context=context))
    return WO_CONF
...
async def al_confirm(update, context):
    ...
    msg = "✅ Локация добавлена." if added else "ℹ️ Такая локация уже есть."
    await show(update, msg, menu_kb())
    context.user_data.clear()
    return MENU


# ═══════════════ ДОБАВИТЬ ВИД ОБОРУДОВАНИЯ (в Справочник) ═══════════════
...
async def ar_confirm(update, context):
    ...
    msg = "✅ Добавлено в справочник." if added else "ℹ️ Такая позиция уже есть."
    await show(update, msg, menu_kb())
    context.user_data.clear()
    return MENU
...
async def inv_part(update, context):
    ...
    names = ENG.location_names()
    await show(update, "По какой локации?", kb(names, extra=[('Все локации', 'all')], context=context))
    return INV_LOC
...
async def inv_loc(update, context):
    ...
    items = ENG.inventory(loc, None, d.get('part'))
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            LOC_PICK: [cq(loc_pick, r'^opt:'), cq(loc_pick, r'^loc_(create|manual)$')],
            # передвижение
