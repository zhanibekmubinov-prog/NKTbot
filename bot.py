...
Экономия токенов:
  • Частые фразы (поступление / передвижение / остатки) разбираются в коде — БЕЗ Claude.
  • Нормализация локаций/размеров/резьбы — в google_sheets.py (нет дублей).
  • Только неразобранные фразы уходят в Claude Haiku (дёшево) с коротким контекстом.

Переменные окружения:
  TELEGRAM_TOKEN, GOOGLE_SHEET_ID, GOOGLE_CREDENTIALS, ANTHROPIC_API_KEY (необязателен).
"""
import asyncio
import json
import logging
...
# порядок важен: длинные варианты раньше коротких
THREADS = ['VAM TOP', 'ВАМ ТОП', 'HCM-1', 'HCM1', 'TMK', 'ТМК', 'VAM', 'ВАМ',
           'EUE', 'ВЫСАДКА', 'NUE', 'ГЛАДКИЙ', 'ГЛАДКАЯ']
...
def _find_size(t):
    ...
    if m:
        return 'ø' + m.group(1)
    m = re.search(r'\b(' + '|'.join(KNOWN_DIA) + r')\b', t)   # голый известный диаметр
    if m:
        return 'ø' + m.group(1)
    return None
...
def try_parse(text):
    """Разбор без Claude. Возвращает (kind, args) или None."""
    t = ' ' + text.lower().strip() + ' '
...
def _need_thread(part, thread):
    """Для НКТ резьба обязательна — если не распознали, отдаём подсказку."""
    return part == 'НКТ' and not thread
...
def run_fast(kind, args):
    if kind == 'inv':
        return _fmt_inv(ENG.inventory(args.get('location'), args.get('status'), args.get('part')))

    if kind == 'move':
        if _need_thread(args['part'], args.get('thread')):
            return ("❓ Для НКТ укажите резьбу (TMK / VAM TOP / HCM-1 / EUE / NUE).\n"
                    "Напр.: «Перевезли {} НКТ {} TMK с {} на {}»".format(
                        args['qty'], args['size'], args['loc_a'], args['loc_b']))
        ...
        th = f" {args['thread']}" if args.get('thread') else ''
        aft = ENG.inventory(args['loc_b'], _status_for(args['loc_b']), args['part'])
        return "🔄 Записал передвижение: {} {}{} — {}→{} ({} шт).\n{}".format(
            args['part'], args['size'], th, args['loc_a'], args['loc_b'], args['qty'], _fmt_inv(aft))

    if kind == 'in':
        if _need_thread(args['part'], args.get('thread')):
            return ("❓ Для НКТ укажите резьбу (TMK / VAM TOP / HCM-1 / EUE / NUE).\n"
                    "Напр.: «Поступило {} НКТ {} TMK от {} на {}»".format(
                        args['qty'], args['size'], args['supplier'], args['location']))
        ...
        return "✅ Записал поступление: {} {}{} — {} шт → {} ({}).\n{}".format(
            args['part'], args['size'], th, args['qty'], args['location'], args['status'], _fmt_inv(aft))
...
SYSTEM = ("Ты помощник склада нефтепромыслового оборудования. Отвечай по-русски, кратко, с эмодзи. "
          ...
          "ВАЖНО: у НКТ всегда есть резьба (TMK, VAM TOP, HCM-1, EUE, NUE) — обязательно передавай "
          "параметр thread при записи НКТ; если пользователь её не указал — уточни одним вопросом, "
          "не записывай без резьбы. У АГРП, Пакер, ЯГ резьбы нет — thread не передавай. "
...
TOOLS = [
    ...
    {"name": "record_movement", "description": "Передвижение А→Б. Для НКТ обязательно thread.",
     "input_schema": {"type": "object",
        ...
        "properties": {"loc_a": {"type": "string"}, "status_a": {"type": "string"},
                       ...
                       "thread": {"type": "string", "description": "Резьба НКТ: TMK/VAM TOP/HCM-1/EUE/NUE"},
                       "qty": {"type": "integer"}}}},
    {"name": "record_incoming", "description": "Поступление от поставщика. Для НКТ обязательно thread.",
     "input_schema": {"type": "object",
...
async def cmd_start(update: Update, _):
    await update.message.reply_text(
        ...
        "• Поступило 50 Пакер ø142 от РД Пакер на База Атамбаева\n"
        "• Сколько НКТ на хранении?\n\n"
        "⚠️ Для НКТ всегда указывайте резьбу (TMK / VAM TOP / HCM-1 / EUE / NUE).\n"
