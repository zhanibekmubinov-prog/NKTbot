...
Новая архитектура: лист «Учет» ПОЛНОСТЬЮ пересобирает Apps Script.
Бот только:
  • пишет строки в журналы «Поступление» / «Передвижение»;
  • добавляет локации в справочник «Локации»;
  • считает остатки на Python (из «Начальные остатки» + журналы) — для ответов в TG.

Никаких формул и copyPaste. Нормализация локаций/номенклатуры — здесь же,
чтобы бот не плодил дубли (это была главная причина «свод считал неверно»).
"""
import re
...
class SheetEngine:
    ...
    def ref(self):
        """{'НКТ':{'sizes':[...],'threads':[...]}, ...} из листа «Справочник»."""
    ...
    def resolve_location(self, name, add_if_new=True):
        """Приводит свободный текст локации к канону из «Локации».
        Возвращает (canonical_name, is_new)."""
        want = _norm(name)
        locs = self.locations()
        # 1) точное / регистронезависимое
        for l in locs:
            if _norm(l['name']) == want:
                return l['name'], False
        # 2) подстрока (в обе стороны) — самое длинное совпадение
        best, best_len = None, 0
        for l in locs:
            ln = _norm(l['name'])
            if want and (want in ln or ln in want):
                if len(ln) > best_len:
    ...
    def normalize_size(self, part, size):
        ...
        if not s:
            return s
        # «142» → «ø142» для видов с диаметром
        if part in ('НКТ', 'Пакер', 'ЯГ') and re.fullmatch(r'\d{2,3}', s):
            s = 'ø' + s
        s = s.replace('O', 'ø').replace('o', 'ø').replace('Ø', 'ø')
        return s

    def normalize_thread(self, thread):
        t = str(thread or '').strip().upper()
        aliases = {'ТМК': 'TMK', 'ВАМ': 'VAM', 'ВАМ ТОП': 'VAM TOP', 'ВЫСАДКА': 'EUE', 'ГЛАДКИЙ': 'NUE'}
    ...
    def record_incoming(self, d, supplier, part, size, thread, location, status, qty, note=''):
        ...
        thread = self.normalize_thread(thread)
    ...
    def _read_init_matrix(self):
        ...
        for c in range(2, len(size)):
            s = (size[c] if c < len(size) else '').strip()
            if not s:
                continue
            cols.append((c, (vid[c] if c < len(vid) else '').strip(), s,
    ...
    def inventory(self, location=None, status=None, part=None):
        ...
        for (st, loc, p, s, t), q in inv.items():
            if q <= 0:                              continue
            if status and _norm(st) != _norm(status): continue
            if part and _norm(p) != _norm(part):    continue
            if loc_want and loc_want not in _norm(loc): continue
