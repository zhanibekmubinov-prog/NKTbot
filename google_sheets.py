...
• проверка наличия позиции (available) — чтобы не двигать/списывать из пустого.

Лист «Учет» пересобирает Apps Script (там же добавлен вычет «Списания»).
"""
...
class SheetEngine:
    ...
    # ── поиск / сопоставление локаций ──
    def find_location(self, name):
        """Точное/вхождение совпадение. Каноническое имя или None."""
        want = _norm(name)
        if not want:
            return None
    ...
    def location_exists(self, name):
        want = _norm(name)
        return any(_norm(n) == want for n in self.location_names())

    def suggest_locations(self, name, limit=3, cutoff=0.55):
        """Нечёткие кандидаты (difflib) по существующим локациям."""
        want = _norm(name)
        scored = []
    ...
    def _inv_here(self, location, status=None, part=None):
        """Остатки строго на данной локации (точное совпадение имени)."""
        return [it for it in self.inventory(None, status, part)
                if _norm(it['location']) == _norm(location)]

    def available(self, location, status, part, size, thread):
        """Сколько числится по конкретной позиции (для проверки при передвижении/списании)."""
        for it in self._inv_here(location, status, part):
            if _s(it['size']) == _s(size) and _s(it.get('thread') or '') == _s(thread or ''):
                return it['qty']
        return 0

    # ── что реально есть на локации (для пошаговых кнопок) ──
    def statuses_at(self, location, part=None, size=None, thread=None):
        found = set()
        for it in self._inv_here(location, None, part):
            if size is not None and _s(it['size']) != _s(size):
                continue
            if thread is not None and _s(it.get('thread') or '') != _s(thread or ''):
                continue
            found.add(it['status'])
        return [s for s in STATUSES if s in found]

    def parts_at(self, location, status):
        seen = []
        for it in self._inv_here(location, status):
            if it['part'] not in seen:
                seen.append(it['part'])
        return [p for p in PARTS_DEFAULT if p in seen] + [p for p in seen if p not in PARTS_DEFAULT]

    def sizes_at(self, location, status, part):
        seen = []
        for it in self._inv_here(location, status, part):
            if it['size'] not in seen:
                seen.append(it['size'])
        return seen

    def threads_at(self, location, status, part, size):
        seen = []
        for it in self._inv_here(location, status, part):
            if _s(it['size']) == _s(size):
                t = it.get('thread') or ''
